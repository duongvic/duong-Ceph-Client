...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/phsnlam/ceph-client/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("ceph-client.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in ceph-client.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "ceph-client.gemspec", /ruby_version = .*">= (.*)"/ => lowest
